...
{
  $stmt = $con->prepare("SELECT `orders`.`id`, `orders`.`orders_number`, `orders`.`customer_id`, `orders`.`product_id`,`orders`.`product_name`, `orders`.`product_quantity`, `orders`.`product_price`, `orders`.`subtotal`, `orders`.`delivery_address`, `orders`.`delivery_datetime`,`orders`.`note_customer`, `orders`.`upload_file`,`orders`.`order_date`, `orders`.`order_status`, `customers`.`name_customer`, `customers`.`email_customer`, `customers`.`phone_customer` FROM `orders` INNER JOIN `customers` ON `orders`.`customer_id` = `customers`.`id` WHERE `orders`.`id` = ? LIMIT 1");
  $stmt->execute(array($id));
  return $stmt->fetch();
}
